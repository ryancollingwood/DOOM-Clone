...
class MapRenderer:
    ...
    def get_bounds(segments: list[tuple[vec2]]):
        ...
        # Comparing the segment's coordinates against each other first (e.g., p0x < p1x)
        # before comparing against the global min/max reduces the total number of conditional
        # checks required per segment, yielding measurable execution speedups.
