...
class BSPTreeTraverser:
    ...
    def _traverse(self, node: BSPNode, x: float, y: float, append_seg_id, visible_sector_bool, append_sector_id):
        ...
        if x * node.splitter_vec_y - y * node.splitter_vec_x < node.splitter_c:
            if front:
                self._traverse(front, x, y, append_seg_id, visible_sector_bool, append_sector_id)
            # Optimization: Track sectors of traversed nodes to ensure all flats
            # in the BSP sub-tree are drawn, even if walls are culled.
            # Optimization: Inlined the sector tracking check to avoid Python function call
            # wrapper overhead. We check the boolean array directly and append if unseen.
            sec_id = node.sector_id
            if not visible_sector_bool[sec_id]:
                visible_sector_bool[sec_id] = True
                append_sector_id(sec_id)

            back_sec_id = node.back_sector_id
            if back_sec_id is not None:
                if not visible_sector_bool[back_sec_id]:
                    visible_sector_bool[back_sec_id] = True
                    append_sector_id(back_sec_id)

            append_seg_id(node.segment_id)
            #
            if back:
                self._traverse(back, x, y, append_seg_id, visible_sector_bool, append_sector_id)
        else:
            if back:
                self._traverse(back, x, y, append_seg_id, visible_sector_bool, append_sector_id)
            #
