...
class Models:
    ...
    def add_wall_model(self, wall_model, segment):
        ...
        if wall_model.wall_type == WallType.PORTAL_MID:
            segment.mid_wall_models.append(wall_model)
        else:
...
class FlatModel:
    # Optimization: __slots__ reduces memory footprint by preventing dynamic __dict__
    # creation for FlatModel objects which are instantiated frequently.
    ...
    def get_outline(self, sector_segments):
        ...
        # removes redundant function calls and arithmetic.
        target_len = len(sector_segments) + 1

        # Optimization: For generating lists with a known fixed size, bypassing dynamic list scaling via
        # `.append()` by pre-allocating the array (`[None] * target_len`) and assigning elements directly
        # in a `for` loop eliminates resizing overhead and yields measurable speedups (e.g., ~15%).
...
class WallModel:
    # Optimization: __slots__ reduces memory footprint by preventing dynamic __dict__
    # creation for WallModel objects which are heavily allocated (thousands per level).
    ...
    def get_quad_mesh(self) -> ray.Mesh:
        ...
        # PyGLM function call overhead (glm.length, glm.normalize) and intermediate Python object
        # allocations (vec3) in this hot path, while utilizing optimized C-level math execution,
