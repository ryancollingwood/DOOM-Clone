...
class DummyCam:
    ...
    def move_opt(self):
        ...
        if math.isfinite(dx):
            old_x = self.pos_3d.x
            new_x = old_x + dx
            self.pos_3d.x = max(-self.MAX_BOUND, min(new_x, self.MAX_BOUND))
            self.target.x += self.pos_3d.x - old_x

        if math.isfinite(dy):
            old_y = self.pos_3d.y
            new_y = old_y + dy
            self.pos_3d.y = max(-self.MAX_BOUND, min(new_y, self.MAX_BOUND))
            self.target.y += self.pos_3d.y - old_y

        if math.isfinite(dz):
            old_z = self.pos_3d.z
            new_z = old_z + dz
            self.pos_3d.z = max(-self.MAX_BOUND, min(new_z, self.MAX_BOUND))
            self.target.z += self.pos_3d.z - old_z
