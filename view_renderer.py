...
class ViewRenderer:
    ...
    def update(self):
        ...
        for seg_id in self.segment_ids_to_draw:
            # optimization: use pre-partitioned lists to avoid O(N) loop and type checks
            segment = self.segments[seg_id]
            self.walls_to_draw.extend(segment.other_walls)
            self.mid_walls_to_draw.extend(segment.mid_walls)
