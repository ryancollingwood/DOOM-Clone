import timeit

s1 = """
class Segment:
    def __init__(self, mid, other):
        self.mid_wall_models = mid
        self.other_wall_models = other

mid1 = {1: 'a', 2: 'b'}
mid2 = {3: 'c'}
other1 = {4, 5}
other2 = {6}

segments = [
    Segment(mid1, other1),
    Segment(mid1, other1),
    Segment(mid2, other2),
    Segment(mid1, other1),
    Segment(mid2, other2)
] * 100

segment_ids_to_draw = list(range(len(segments)))

mid_walls_to_draw = {}
walls_to_draw = set()

def update():
    walls_to_draw.clear()
    mid_walls_to_draw.clear()

    processed_mid = set()
    processed_other = set()

    for seg_id in segment_ids_to_draw:
        seg = segments[seg_id]

        mid_id = id(seg.mid_wall_models)
        if mid_id not in processed_mid:
            mid_walls_to_draw.update(seg.mid_wall_models)
            processed_mid.add(mid_id)

        other_id = id(seg.other_wall_models)
        if other_id not in processed_other:
            walls_to_draw.update(seg.other_wall_models)
            processed_other.add(other_id)
"""

s2 = """
class Segment:
    def __init__(self, mid, other):
        self.mid_wall_models = mid
        self.other_wall_models = other

mid1 = {1: 'a', 2: 'b'}
mid2 = {3: 'c'}
other1 = {4, 5}
other2 = {6}

segments = [
    Segment(mid1, other1),
    Segment(mid1, other1),
    Segment(mid2, other2),
    Segment(mid1, other1),
    Segment(mid2, other2)
] * 100

segment_ids_to_draw = list(range(len(segments)))

mid_walls_to_draw = {}
walls_to_draw = set()

def update():
    walls_to_draw.clear()
    mid_walls_to_draw.clear()

    processed_mid = set()
    processed_other = set()

    for seg_id in segment_ids_to_draw:
        seg = segments[seg_id]

        mid_models = seg.mid_wall_models
        mid_id = id(mid_models)
        if mid_id not in processed_mid:
            mid_walls_to_draw.update(mid_models)
            processed_mid.add(mid_id)

        other_models = seg.other_wall_models
        other_id = id(other_models)
        if other_id not in processed_other:
            walls_to_draw.update(other_models)
            processed_other.add(other_id)
"""

print(timeit.timeit("update()", setup=s1, number=10000))
print(timeit.timeit("update()", setup=s2, number=10000))
