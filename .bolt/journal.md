...
### 2024-06-25: Optimize bounding box calculations with conditional logic reordering
**Problem:** In `MapRenderer.get_bounds`, evaluating both segment endpoints independently against the global minimum and maximum variables required 4 complete conditional checks per axis per segment (e.g., `if p0x < x_min`, `if p1x < x_min`, `if p0x > x_max`, `if p1x > x_max`).
**Optimization:** Reordered the logic to first compare the segment's coordinates against each other (`if p0x < p1x`) before performing global bounds checks. This ensures that the smaller coordinate is only checked against the global minimum, and the larger is only checked against the global maximum, reducing the total condition evaluations required.
**Impact:** `timeit` synthetic benchmarking demonstrated that reducing the number of conditional checks required per segment yields execution speedups for calculating bounding boxes (execution time drops from ~1.31s down to ~1.15s for 2000 loops of 5000 segments, achieving roughly a ~12% speedup).
