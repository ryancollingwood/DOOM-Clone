...
### 2024-06-17: Optimize `ViewRenderer.draw` to only draw visible flats
**Problem:** The `ViewRenderer.draw` method iterated over all global sectors (`self.sectors`) to draw flats (floors and ceilings), regardless of whether they were visible or behind walls. This resulted in significant overdraw and unnecessary renderer calls (O(Total Sectors)).
**Optimization:** Replaced the global sector iteration with a dynamic `self.visible_sector_ids` tracking set. During `ViewRenderer.update()`, as visible segments are processed (derived from BSP traversal), their corresponding `sector_id` and `back_sector_id` are added to the set. The set is cleared at the start of every frame and the `visible_ids_add = self.visible_sector_ids.add` method is cached as a local variable to prevent `LOAD_ATTR` overhead. `ViewRenderer.draw` now only iterates over this culled set (O(Visible Sectors)).
**Impact:** Benchmarks via `timeit` across 10,000 iterations over dummy data (simulating visible vs hidden sectors out-of-bounds) indicated rendering total time dropped from ~4.4s down to ~2.6s, achieving over a ~40% speedup by eliminating redundant `ray.draw_model` calls.
