...
### 2024-06-05: Minimize Python Object Allocation in PyGLM array processing
**Problem**: The `get_polygon_mesh` and `get_quad_mesh` methods generate large numbers of `glm.array` inputs. Previously, `get_polygon_mesh` built a full list of `glm.vec2` objects via comprehension, and then iterated over it *again* to create negative vectors if it was mapping a ceiling, creating unnecessary intermediate objects. `get_quad_mesh` created redundant temporary tuples and lists before passing to PyGLM (e.g. `[normal] * vertex_count` and unpacking tuples).
**Optimization**: Refactored `FlatModel.get_polygon_mesh` to hoist the ceiling conditional *outside* the list comprehension, so `glm.vec2` objects are only allocated once. In `WallModel.get_quad_mesh`, manually constructed small, fixed-size lists (e.g. `[normal, normal, normal, normal]`) and passed explicit scalars directly to `glm.vec2` to bypass temporary object creation overheads completely.
**Impact**: Synthetic benchmarks using `timeit` for the array generation steps demonstrated execution time roughly halving for both `FlatModel` mesh generation and `WallModel` geometric allocations, speeding up the core rendering generation hotpaths by dodging unnecessary list unpacking and object allocation.
