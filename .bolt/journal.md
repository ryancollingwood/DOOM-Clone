...
### 2024-06-03: WallModel.get_quad_mesh intermediate vec3 allocations
**Problem**: The `WallModel.get_quad_mesh` method allocates intermediate `vec3` objects to compute the difference between vertices (`delta = vec3(x1, 0, z1) - vec3(x0, 0, z0)`), and then evaluates `width = glm.length(delta)`. This incurs unnecessary memory allocation and function call overhead for operations that are otherwise simple scalar math.
**Optimization**: Manually calculated `dx = x1 - x0` and `dz = z1 - z0`, and inlined the magnitude calculation as `width = (dx*dx + dz*dz)**0.5`. Used `dx` and `dz` to compute the normal. This avoids generating two intermediate `vec3` objects per wall mesh and speeds up the tight mesh generation logic.
**Impact**: Using a simple synthetic benchmark (`timeit` over 1,000,000 executions), execution time dropped from 1.22s to 0.61s. This is roughly a 2x speedup for this specific geometric computation path.
