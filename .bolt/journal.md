...
### 2024-08-18: Optimize `MapRenderer.get_bounds` min/max comparisons
**Problem:** In `MapRenderer.get_bounds`, determining the bounding box for an array of line segments required comparing both the start (`p0`) and end (`p1`) coordinates against the global running minimums and maximums (`x_min`, `x_max`, `y_min`, `y_max`). This resulted in 4 independent conditional checks per coordinate axis, per loop iteration.
**Optimization:** Updated the logic to first compare the internal segment points against each other (e.g., `p0.x < p1.x`). By establishing the local minimum and maximum between the pair upfront, we ensure that only the smaller value needs to be checked against the global minimum, and only the larger value needs to be checked against the global maximum.
**Impact:** `timeit` profiling of the bounding box logic over 10,000 runs using 1,000 synthetic segments indicated the execution time dropped from roughly 1.63s to 1.43s. This reduces total comparison checks per loop, yielding approximately a 12% speedup for this specific method.
