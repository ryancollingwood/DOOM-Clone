...
### 2024-06-14: Optimize `ViewRenderer.update` tracking using a single integer segment ID
**Problem**: The `ViewRenderer.update` method iterates over potentially thousands of visible BSP split segments. Previously, it evaluated and tracked the identity of the `mid_wall_models` and `other_wall_models` collections using the `id()` function in multiple separate tracking sets to avoid redundant updates. This approach still introduced overhead from calling `id()` and maintaining multiple hash sets per segment type.
**Optimization**: Added a unique integer `seg_id` to the `Segment` class, assigned sequentially during level parsing. Since the BSP split segments are shallow copies of these original segments, the `seg_id` represents a strict 1:1 mapping with the parent segment. The `ViewRenderer.update` loop was refactored to use a single `processed_ids` set tracking this integer `seg_id`, bypassing `id()` function calls and consolidating dictionary lookup and insert operations.
**Impact**: Profiling in isolated tight loops indicated that bypassing `id()` and using a single tracking set reduces execution time from ~5.7s to ~2.6s (over 10000 runs mapping 500 duplicated sets), yielding roughly a **~54% performance speedup** in collection state tracking.
