...
### 2024-06-25: Optimize `FlatModel.get_polygon_mesh` by evaluating conditional list comprehension once
**Problem:** In `models.py`'s `FlatModel.get_polygon_mesh`, the code unconditionally evaluated a list comprehension `[glm.vec2(v) for v in sector_verts]` and conditionally evaluated a second list comprehension using the first list `tex_coords if self.is_floor else [glm.vec2(v.x, -v.y) for v in tex_coords]`. This forced the instantiation and iteration of an intermediate list.
**Optimization:** Replaced the double evaluation with a single conditional list comprehension `[glm.vec2(v) for v in sector_verts] if self.is_floor else [glm.vec2(v[0], -v[1]) for v in sector_verts]`, avoiding the intermediate list wrapper generation entirely.
**Impact:** `timeit` testing over 10,000 iterations indicates that using a single conditional list comprehension drops evaluation time from ~2.86s to ~1.62s, representing roughly a ~43% execution speed improvement.
