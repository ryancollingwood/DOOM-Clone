...
### 2024-06-25: Optimize `MapRenderer.get_bounds` coordinate comparisons
**Problem:** In `MapRenderer.get_bounds`, each segment's coordinates were compared independently against the global min and max bounds, resulting in four independent conditional checks per coordinate pair (e.g., `if p0x < x_min`, `if p1x < x_min`, etc.), which introduced unnecessary branching evaluation overhead in a hot path.
**Optimization:** Grouped the coordinate checks by first comparing the segment's own coordinates against each other (e.g., `if p0x < p1x:`). This guarantees that the smaller coordinate is only checked against the global minimum, and the larger coordinate is only checked against the global maximum, reducing the total number of conditional evaluations per segment.
**Impact:** `timeit` synthetic benchmarking over 1000 items and 10000 iterations demonstrated that the execution speed drops from ~1.55s to ~1.40s, delivering approximately a ~10% performance improvement by minimizing branching overhead.
