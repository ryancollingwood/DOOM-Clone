...
### 2024-06-23: Optimize `ViewRenderer.draw` by pre-evaluating boolean properties
**Problem:** In `ViewRenderer.draw()`, walls are dynamically tinted based on whether they should be shaded or not. Inside the main rendering loop over `self.walls_to_draw` and `self.mid_walls_to_draw` (potentially thousands of entries), the boolean `shade_tint if wall.is_shaded else screen_tint` logic was being continuously evaluated. Additionally, attributes like `self.flat_models` were repetitively looked up.
**Optimization:** Taking advantage of the fact that booleans evaluate as `0` and `1` in Python, we created a static tuple `tints = (screen_tint, shade_tint)` prior to the loops. The loops then index this tuple `tints[wall.is_shaded]` natively via C-backed indexing, avoiding Python `LOAD_GLOBAL`, `POP_JUMP_IF_FALSE`, and branch resolution overhead in hot loops. Similarly, hoisted `self.flat_models` out of its loop.
**Impact:** `timeit` profiling of the isolated rendering logic with synthetic loop scaling proved a ~18-20% execution speedup by dodging branching inline evaluation syntax and static lookups.
