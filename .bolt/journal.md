...
### 2024-06-22: Optimize FlatModel and WallModel memory usage using `__slots__`
**Problem:** The `FlatModel` and `WallModel` classes are instantiated numerous times when a level is loaded (each sector generates floor/ceiling `FlatModel`s, and each segment generates `WallModel`s). Because standard Python classes create a dynamic dictionary (`__dict__`) to store attributes, this introduces measurable memory allocation overhead and lookup costs during object instantiation, which hurts engine load times.
**Optimization:** Added `__slots__` explicitly listing the class attributes (`'engine', 'textures', 'sector_segments', 'sectors', 'sector_id', 'sector', 'is_floor', 'model'` for `FlatModel`, and `'engine', 'textures', 'segment', 'sectors', 'wall_type', 'is_shaded', 'model'` for `WallModel`). This prevents Python from creating the underlying dictionary for instances, strictly defining memory layout.
**Impact:** Using synthetic `timeit` benchmarks for instantiating 1,000,000 dummy model objects:
- `WallModel`: Execution time dropped from ~0.330s to ~0.309s (~6% speedup).
- `FlatModel`: Execution time dropped from ~0.339s to ~0.325s (~4% speedup).
These improvements compound to lower the peak memory footprint and speed up the geometry loading pipeline during level initialization.
