...
### 2024-06-17: Optimize `ViewRenderer.draw` flat rendering via visible sector tracking\n**Problem:** The `ViewRenderer.draw` method iterated over all `self.sectors` to draw floors and ceilings, resulting in redundant rendering calls for sectors that were completely out of the camera's view, unnecessarily burdening the GPU pipeline.\n**Optimization:** Introduced a `self.visible_sector_ids` set in `ViewRenderer`. During `ViewRenderer.update`'s segment traversal (which already determines visible geometry), dynamically populated this set with the `sector_id` and `back_sector_id` of visible segments. Swapped `self.sectors` to `self.visible_sector_ids` in `ViewRenderer.draw()`'s flat rendering loop.\n**Impact:** Profiling with `profile_test.py` across 1,000 frames showed a massive reduction in `raylib._raylib_cffi.DrawModel` calls (from 57,000 to 18,000), slashing total drawing time from ~9.7s to ~2.6s, achieving roughly a ~73% speedup in drawing operations by culling invisible geometry.
