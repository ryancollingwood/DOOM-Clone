...
### 2024-06-02: Optimize FlatModel.get_outline with O(N) graph traversal
**Problem**: `FlatModel.get_outline` calculates the sequential vertex outline from unordered sector segments. Its `while` loop implementation iteratively searched the entire remaining segment list in O(N) time for every vertex to find connected edges, resulting in roughly O(N^2) complexity. For levels with highly complex, multi-segmented sector geometry, this caused significant initialization and map load delays.
**Optimization**: Replaced the naive list search with an explicit graph builder step that generates a dictionary mapping each vertex directly to its connected neighbors (`graph = {}`). Then traversed the graph starting from the first vertex, resolving sequential connections with O(1) dictionary lookups.
**Impact**: Using `timeit` for an unordered 200-segment polygonal sector outline over 1000 repetitions, the execution time dropped from ~6.03s down to ~0.15s, yielding roughly a **40x speedup** on outline generation.
