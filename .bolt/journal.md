...
### 2024-06-01: Optimize Render Tracking collections with Lists
**Problem**: The `mid_wall_models` within `Segment` and the frame tracking collection `mid_walls_to_draw` in `ViewRenderer` were using ordered `dict` objects. Deduplication was already handled by tracking the shared segment `id()` in `processed_mid`, making the dictionary key assignment redundant. Because `mid_walls_to_draw.update()` iterates keys and performs hash map checks, it added unnecessary overhead during BSP traversal rendering loops.
**Optimization**: Changed `Segment.mid_wall_models` to initialize as a `list` instead of a `dict`, mapping updates in `Models.add_wall_model` to `.append()`. Refactored `ViewRenderer.update` to initialize `self.mid_walls_to_draw` as a `list` and use `.extend()`, which is much faster than `dict.update()` because it skips hashing and collision logic. The `draw` loop now also iterates without the slower `.values()` method call.
**Impact**: Profiling 1,000,000 simulated render loops with 5 wall segments demonstrated execution time dropping from ~1.52 seconds to ~1.23 seconds using lists instead of dictionaries, achieving a ~19% speedup while fully maintaining Painter's algorithm transparency (insertion order). Memory overhead per wall addition is also reduced.
