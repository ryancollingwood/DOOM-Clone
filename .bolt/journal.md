...
\n### 2026-09-13: Optimize `MapRenderer.get_bounds` conditional evaluation overhead\n**Problem:** In `MapRenderer.get_bounds`, iterating over a large number of segments to find the global bounding box involves 8 independent conditional evaluations per segment. Since the coordinates of the segment (`p0` and `p1`) themselves have a guaranteed mathematical relationship, many of these checks are redundant.\n**Optimization:** Optimize `MapRenderer.get_bounds` by comparing segment coordinates to each other before checking against global min/max bounds, which reduces conditional evaluation overhead.\n**Impact:** `timeit` synthetic benchmarking showed a speed improvement from ~1.65s to ~1.45s for calculating bounds of 1000 segments over 10000 iterations (an ~12% speedup).
