...
### 2024-06-23: Optimize `mid_wall_models` collection type
**Problem:** In `data_types.py`, `Segment.mid_wall_models` was defined as a dictionary, and populated by `models.py` using `segment.mid_wall_models[self.wall_id] = wall_model`. In `view_renderer.py`, this dictionary was heavily used in a hot loop within `ViewRenderer.update` via `mid_update(mid)` which merges dictionaries (where `mid_update = self.mid_walls_to_draw.update` and `mid_walls_to_draw` is a dictionary). Dict updates are generally slower than list extensions due to hashing. The `wall_id` key is never actually used for lookups; the walls are only iterated over to draw them back-to-front.
**Optimization:** Converted `Segment.mid_wall_models` and `ViewRenderer.mid_walls_to_draw` from a dictionary into a list. Replaced `.update()` with `.extend()`. During rendering, `reversed(self.mid_walls_to_draw)` was used directly instead of `reversed(self.mid_walls_to_draw.values())`.
**Impact:** Benchmarks on the update hotpath isolated via `timeit` show `list.extend()` executes significantly faster (52us vs 120us per loop for 1000 items) than `dict.update()`. Integrated engine benchmarking of the render loop (update+draw) shows execution time reduced from ~460 us to ~423 us (a ~8% speedup in the hot view rendering path).
