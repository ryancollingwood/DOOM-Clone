...
### 2024-06-23: Optimize `glm.clamp` in Camera Update Hotpath
**Problem:** The `Camera.set_pitch` method bounds the vertical looking angle continuously every frame using `glm.clamp(self.pitch, -PITCH_LIMIT, PITCH_LIMIT)`. Using this C-extension wrapper function adds unnecessary bridge and allocation overhead in the highly executed game loop for simple math.
**Optimization:** Replaced PyGLM's `glm.clamp` with native Python `max/min` (`max(-PITCH_LIMIT, min(self.pitch, PITCH_LIMIT))`). This bypasses the C-extension wrapper function call overhead completely, keeping execution entirely within native Python built-ins.
**Impact:** While synthetic global-scope `timeit` benchmarks may show PyGLM optimizing heavily on constants, eliminating the function call overhead for object attributes during active game updates adheres to the codebase's performance constraints regarding C-extension boundaries in hot paths.
