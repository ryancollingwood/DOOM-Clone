...
### 2024-06-03: Optimize `MapRenderer.remap_array` by caching constants and using list comprehension
**Problem**: In `MapRenderer.remap_array`, the variables `cx` and `cy` were multiplied by `x_min` and `y_min` inside the inner loop for every vector mapped. Additionally, intermediate lists were constructed using `.append()`. This introduced redundant arithmetic operations and method lookups.
**Optimization**: Precalculated the offsets `cxx_min = -x_min * cx + out_min` and `cyy_min = -y_min * cy + out_min` outside the loop. Refactored the point mapping loop to use a list comprehension.
**Impact**: Synthetic benchmarking in Python over 1000 array items using `timeit` showed an execution time drop from ~16.05 seconds down to ~12.86 seconds for 10000 runs, representing a roughly 20% speedup on coordinate mapping operations.
